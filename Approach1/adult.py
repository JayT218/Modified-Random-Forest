...
def worst_forest():
    ...
    for forest in range(N_forest):
        ...
        for element in range(len(y_test_list)):
            if y_predict[forest][element][0] == y_test_list[element][0]:
                score += 1
...
def update_forest(fid, fid_score):
    ...
    cnt = 0
    for element in range(len(y_test_list)):
        if y_predict[fid][element][0] == y_test_list[element][0]:
            cnt += 1
...
def voting():
    ...
    for column in range(len(y_test_list)):
        ...
        for row in range(N_forest):
            ...
            if y_predict[row][column][0] not in dicvote:
                dicvote[y_predict[row][column][0]] = 1
            else:
                dicvote[y_predict[row][column][0]] += 1
